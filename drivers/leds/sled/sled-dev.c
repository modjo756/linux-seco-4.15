...
static long sled_dev_ioctl (struct file *file, unsigned int cmd, unsigned long arg) {
	...
	switch (cmd) {
		case SLED_NAME_GET:
			...
			mutex_unlock (&sled->ops_lock);
			if (copy_to_user (uarg, name, sizeof (char) * SLED_DEVICE_NAME_SIZE))
				err = -EFAULT;
		...
		case SLED_TYPE_GET:
			...
			err = 0;
			type = sled->led_type;
			if (copy_to_user (uarg, &type, sizeof (type)))
				err = -EFAULT;
		...
		case SLED_ENABLE_SET:
			...
			if (copy_to_user ((void __user *)uarg, &enable, sizeof(enable)))
				return -EFAULT;
		...
		case SLED_ENABLE_GET:
			...
			if (copy_to_user (uarg, &enable, sizeof (enable)))
				err = -EFAULT;
		...
		case SLED_DUTY_SET:
			...
			if (copy_from_user (&duty, uarg, sizeof(duty)))
				return -EFAULT;

			sled_store_duty (sled, duty);

			if (copy_to_user ((void __user *)uarg, &duty, sizeof(duty)))
				return -EFAULT;
		...
		case SLED_DUTY_GET:
			...
			if (copy_to_user (uarg, &duty, sizeof (duty)))
				err = -EFAULT;
		...
		case SLED_MAX_DUTY_GET:
			...
			if (copy_to_user (uarg, &max_duty, sizeof (max_duty)))
				err = -EFAULT;
		...
		case SLED_PERIOD_SET:
			...
			if (copy_from_user (&period, uarg, sizeof(period)))
				return -EFAULT;

			sled_store_period (sled, period);

			if (copy_to_user ((void __user *)uarg, &period, sizeof(period)))
				return -EFAULT;
		...
		case SLED_PERIOD_GET:
			...
			if (copy_to_user (uarg, &period, sizeof (period)))
				err = -EFAULT;
		...
		case SLED_MAX_PERIOD_GET:
			...
			if (copy_to_user (uarg, &max_period, sizeof (max_period)))
				err = -EFAULT;
		...
		case SLED_MIN_PERIOD_GET:
			...
			if (copy_to_user (uarg, &min_period, sizeof (min_period)))
				err = -EFAULT;
		...
		case SLED_DELAY_SET:
			...
			if (copy_from_user (&delay, uarg, sizeof(delay)))
				return -EFAULT;

			sled_store_delay (sled, delay);

			if (copy_to_user ((void __user *)uarg, &delay, sizeof(delay)))
				return -EFAULT;
		...
		case SLED_DELAY_GET:
			...
			if (copy_to_user (uarg, &delay, sizeof (delay)))
				err = -EFAULT;
		...
		case SLED_MAX_DELAY_GET:
			...
			if (copy_to_user (uarg, &max_delay, sizeof (max_delay)))
				err = -EFAULT;
